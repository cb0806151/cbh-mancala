...
(async () => {
  ...
  const getRandomMove = (currentTurnIndex) => (Math.floor(Math.random() * 6) + (currentTurnIndex == 0 ? 6 : 0)) + 1

  const DefaultActions = {
    gameEnds: async (winIndex, state) => {
      console.log(`Alices final score: ${stdlib.bigNumberToNumber(state.points[0])}`)
      console.log(`Bobs final score: ${stdlib.bigNumberToNumber(state.points[1])}`)
