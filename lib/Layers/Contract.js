...
import * as backend from '../contractv2/build/index.main.mjs'
...
export const gameEnds = (messagesIndex, state) => {
    const closingMessage = `${gameEndMessages[messagesIndex]} Alice finishes with a final score of ${reach.bigNumberToNumber(state.points[0])} and Bob with a final score of ${reach.bigNumberToNumber(state.points[1])}.`
...
export const getMove = async (totalBoard, currentTurnIndex) => {
    ...
    console.log(reach.bigNumberToNumber(currentTurnIndex) === 0 ? `Alice has ${formattedBoard[13]} point(s)` :  `Bob has ${formattedBoard[6]} point(s)`)
    const board = formattedBoard.slice(0, 6).concat(formattedBoard.slice(7,13))
    console.log(board, formattedBoard, points)
    ...
    console.log(updatedMoveHouseIndex)
